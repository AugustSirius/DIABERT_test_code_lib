...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match process_library_fast(file_path) {
        Ok(library) => {
            let _precursor_id_list = vec!["AAAAAAALQAK2"];

            println!("\n总共加载了 {} 条记录", library.len());
            
            // 打印前5条记录
            println!("\n前5条记录（选定列）:");
            for (i, record) in library.iter().take(5).enumerate() {
                println!("记录 {}:", i + 1);
            ...
            // 验证数据处理的正确性
            println!("验证数据处理:");
            println!("- 第一条记录的所有核心字段:");
            if let Some(first) = library.first() {
                println!("  PrecursorMz: {}", first.precursor_mz);
                println!("  ProductMz: {}", first.product_mz);
                println!("  LibraryIntensity: {}", first.library_intensity);
                println!("  ProteinID: {}", first.protein_id);
                println!("  Gene: {}", first.gene);
            }
            
            // 导出结果用于比较
            export_to_csv(&library, "rust_output.csv")?;
        }
        Err(e) => {
            eprintln!("错误: {}", e);
            return Err(e);
