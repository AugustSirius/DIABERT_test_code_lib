...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // ================================ PARALLEL PRECURSOR PROCESSING ================================
    println!("\n========== PARALLEL PRECURSOR PROCESSING ==========");
    println!("Processing {} precursors using {} CPU cores", precursor_id_list.len(), num_threads);
    
    let parallel_start = Instant::now();
    
    // Process all precursors in parallel
    let results: Vec<PrecursorResult> = precursor_id_list
        .par_iter()
        .map(|precursor_id| {
            process_single_precursor(
                precursor_id,
                &library_records,
                &ms1_indexed,
                &finder,
                ...
                eprintln!("Error processing {}: {}", precursor_id, e);
                PrecursorResult {
                    id: precursor_id.to_string(),
                    processing_time: 0.0,
                    dataframe: None,
    ...
    let total_parallel_time = parallel_start.elapsed().as_secs_f32();
    ...
    for result in &results {
        if let Some(df) = &result.dataframe {
            ...
            // let output_path = format!("{}_final_dataframe.csv", result.id);
        ...
        println!("{}: {:.3}s - {}", 
                 ...
                 if result.dataframe.is_some() { "Success" } else { "Failed" });
    }
    ...
    println!("\nTiming Statistics:");
    println!("  - Total parallel processing time: {:.3} seconds", total_parallel_time);
    println!("  - Average time per precursor: {:.3} seconds", total_parallel_time / precursor_id_list.len() as f32);
    println!("  - Sum of individual processing times: {:.3} seconds", total_individual_time);
    println!("  - Parallel speedup: {:.2}x", total_individual_time / total_parallel_time);
    println!("  - CPU utilization: {:.1}%", (total_individual_time / total_parallel_time) / num_threads as f32 * 100.0);
